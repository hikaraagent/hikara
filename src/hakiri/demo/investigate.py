"""demo investigate: walks the full pipeline on one synthetic block.
...
def run_demo_investigate() -> None:
    ...
    for s in swaps:
        c.print(
            f"  block {s.block_number} idx {s.tx_index}  "
    ...
    c.print(f"  block verdict: {result.verdict.value}")
